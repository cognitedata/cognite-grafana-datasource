...
export class CogniteQuerySection extends PureComponent<Props, State> {
    ...
    render() {
        ...
        const granularityTooltip: PopoverContent = 'The granularity of the aggregate values. Valid entries are: \'day/d, hour/h, minute/m, second/s\'. Example: 12hour.';
        const labelTooltip: PopoverContent = 'Set the label for the timeseries. Can also access timeseries properties via {{property}}. Eg: {{description}}-{{metadata.key}}';
        ...
        return (
            <div className="gf-form-inline">
                ...
                <AsyncSelect loadOptions={query => this.getOptions(query, 'Timeseries')} onChange={this.changeTimeseries} loadingMessage={() => 'loading...'} noOptionsMessage={() => 'No timeseries found'} defaultOptions={true} defaultValue={queryTarget.target} backspaceRemovesValue={true} isClearable={true}/>
                ...
                </div>
                {
                    ...
                    ? <FormField label='Granularity' onBlur={this.changeGranularity} labelWidth={7.5} inputWidth={8} defaultValue={queryTarget.granularity} tooltip={granularityTooltip} placeholder={'default'}></FormField>
                    : null
                }
                <div className="gf-form gf-form--grow">
                    <FormField className='gf-form--grow' inputWidth={0} label='Label' onBlur={this.changeLabel} defaultValue={queryTarget.label} tooltip={labelTooltip}/>
                </div>
            </div>
        );
    }
