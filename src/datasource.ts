...
> {
  ...
  private replaceVariablesInTarget(
    ...
    const [
      ...
      [
        ...
        // Instance-reference variables become GraphQL syntax before Grafana
        // pastes the remaining variables in verbatim.
        flexibleDataModellingQuery?.graphQlQuery &&
          interpolateInstanceRefs(
            flexibleDataModellingQuery.graphQlQuery,
            this.templateSrv,
            scopedVars,
          ),
    ...
    const templatedflexibleDataModellingQuery = flexibleDataModellingQuery && {
      flexibleDataModellingQuery: {
        ...
        // Interpolated like every other tab's label; its {{field}} tokens are
        // resolved later, against the response. Left out when unset, so a query
        // that never had a label does not gain an empty one.
        ...(flexibleDataModellingQuery.label
          ? {
            label: this.replaceVariable(
              flexibleDataModellingQuery.label,
              scopedVars,
            ),
          }
  ...
  async metricFindQuery(
    { query, valueType, queryType, graphqlQuery, dataModel, displayField }:
      VariableQueryData,
  ): Promise<MetricDescription[]> {
    // Deliberately ungated by the feature flags: a dashboard saved before an admin
    // changed them must keep resolving its variables.
    if (queryType === "graphql") {
      return metricFindGraphqlQuery(this.connector, this.templateSrv, {
        graphqlQuery,
        dataModel,
        valueType,
        displayField,
  ...
  /**
   * Runs a variable's GraphQL query and hands back the untouched response, for the
   * query editors' preview panes.
   */
  async runGraphqlQuery(args: {
    graphqlQuery: string;
    dataModel: { space: string; externalId: string; version: string };
  }): Promise<{ data?: any; errors?: any }> {
    return runGraphqlQuery(this.connector, this.templateSrv, args);
  }
